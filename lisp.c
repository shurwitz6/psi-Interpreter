...
// --------------- constructors ---------------------
// Create lisp number from signed integer
...
// Create empty resizable lisp list 
pval* pval_list() {
...
// Create lisp builtin function obj
...
pval* lisp_eval_list(pval* list, env* e) {
  res_array* d = (res_array*)list->data;
  pval* args = pval_list();
  for (int i = 0; i < d->size; i++) {
    pval* val = lisp_eval(d->items[i], e);
...
pval* lisp_apply(pval* func, pval* args) {
...
pval* lisp_eval(pval* pv, env* e) {
  ...
  case PVAL_LIST: {
    ...
    // Apply and return
    pval* result = lisp_apply(func, args1);
    pval_delete(func);
    pval_delete(args1);
    return result;
  }
...
pval* eval_if(pval* list, env* e) {
  ...
  if (d->size < 3 || d->size > 4)
    return pval_error(pval_symbol("arity-error"));
  pval* cond = lisp_eval(d->items[1], e);
  if (cond->type == PVAL_ERROR)
    return cond;
  // lisp Spec: only #f is falsy; all other values are truthy
  bool is_false = (cond->type == PVAL_BOOL && !*((bool*)cond->data));
  pval_delete(cond);
  if (is_false)
    // lisp Spec: "In the arity-2 case, behaves as if else-form were given as
    // #f."
    return (d->size == 4) ? lisp_eval(d->items[3], e) : pval_bool(false);
  return lisp_eval(d->items[2], e);
}
...
pval* eval_def(pval* list, env* e) {
  ...
  if (d->items[1]->type != PVAL_SYMBOL)
    return pval_error(pval_symbol("type-error"));
  pval* val = lisp_eval(d->items[2], e);
  if (val->type == PVAL_ERROR)
    return val;
